...
const appGamesNames = [
    {
        ...
        desc:['Choose a Topic: Select a subject or topic for your quiz.',
...
},
{
    ...
    desc:[`Define Your Topic: Input your quiz topic or subject area.`,
...
{appGamesNames.map((game)=>(
    ...
    >
        <CardHeader className='relative  h-full w-full flex flex-col items-center justify-center  '>
            ...
            <CardDescription>
                <ul className='p-5'>
                    {game.desc?.map((des,index)=>(<li key={index} className='text-base  '> {des}</li>))}
                </ul>
            </CardDescription>
