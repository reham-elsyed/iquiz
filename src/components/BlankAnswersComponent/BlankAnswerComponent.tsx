...
function BlankAnswerComponent({answer, input,pkeyWords}: Props){
  ...
  return (
    <div className="flex justify-start w-full mt-4">
        <h1 className='text-xl font-semibold'> {answerWithBlanks.split("_____").flatMap((part ,index)=>
            [part,  index < answerWithBlanks.split("_____").length - 1 &&
             <input
             ref={(el) => {
                if (el) {
